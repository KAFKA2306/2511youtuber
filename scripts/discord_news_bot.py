...
def workflow_command(settings: dict[str, Any], query: str) -> list[str]:
    ...
    if not path.is_absolute():
        ...
        if not resolved:
            home_candidate = Path.home() / ".local/bin" / binary
            project_candidate = resolve_path(binary)
            resolved = str(home_candidate) if home_candidate.exists() else (
                str(project_candidate) if project_candidate.exists() else binary
            )
        command[0] = resolved if resolved else binary
